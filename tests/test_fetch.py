...
# Regions that produce no output are a normal, expected outcome -- fetch is
# typically pointed at many intervals, so it warns and moves on rather than
# failing. Maps region id -> the warning code fetch should report:
#   06-noalign      scaffold is in the index, but no block overlaps the region
#   09-missingchrom the BED names a scaffold the index doesn't have at all
# Both warn and exit 0. (09-missingchrom used to exit non-zero, but only as a
# side effect of the no-overlap count thresholds, which were being computed
# from the wrong summary column; those are advisory now.)
WARN_ONLY_REGIONS = {
    "06-noalign": "no-overlap",
    "09-missingchrom": "missing-scaffold",
}
...
def test_maf_fetch_region(chrom, start, end, region_id, tmp_path):
    ...
    if region_id in WARN_ONLY_REGIONS:
        expected_warning = WARN_ONLY_REGIONS[region_id]
        ...
        assert expected_warning in result.stderr, f"Expected a {expected_warning} warning for region {region_id}"
        return

    assert os.path.getsize(expected_file) > 0, (
        f"{expected_file} is empty, but every remaining region is expected to produce output; "
        f"regions that legitimately produce none belong in WARN_ONLY_REGIONS"
    )
...
def test_scaffold_subdirs_groups_output_by_scaffold(tmp_path):
    ...
    # chr1 has 3 regions with real output (06-noalign overlaps nothing, so no
    # file); chr4 has 2; chr2/chr3/chrX have 1 each. chrZ (09-missingchrom)
    # isn't in the index, so it produces no file and no chrZ/ subdirectory.
